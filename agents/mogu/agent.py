...
root_agent = Agent(
    ...
    instruction=(
        ...
        "spot_id は prefetch にある UUID をそのままコピーする（例: 22222222-2222-4222-8222-222222222303）。"
        "短い仮名や <spot_id> プレースホルダは絶対に使わない。\n"
        "マーカーは機械可読でユーザーには表示されないため、"
        "本文でマーカーの存在に触れない。"
        "prefetch ブロックに無い spot_id / place_id を捏造しない。"
        "本文の提案とマーカーの店は必ず一致させる。"
        "マーカー行を付けられない店名は本文にも書かない。"
        "子エージェント（Ken / Aoi）の応答にマーカー行が含まれる場合は、"
        "本文を自分の言葉に書き直しても、そのマーカー行を最終応答の末尾に"
        "一字も変えずそのまま転記する（#313）。\n"
        "\n"
        "## 禁止（ユーザー向け出力）\n"
