...
Pub/Sub ──OIDC push──▶ Cloud Run service「incident-agent-ingest」(非公開)
                  ...
                  一次切り分け結果(仮説・重大度・推奨アクション・根拠・確信度)
                     ...
                     └─ ops.outboxへSlack/GitHub出力意図を保存
                          └─ dispatcher Jobが依存解決済み行をCloud Tasksへ
                               └─ private workerが冪等送信
...
CREATE INDEX ON ops.outbox (depends_on);
...
- 出力フォーマット(固定): `仮説(1文) / 根拠(参照したログ・メトリクス・類似事例) / 重大度 / 推奨アクション(人間向け) / 確信度`。仮説・根拠の説明・推奨アクションは日本語で生成し、ログ原文・メトリクス名・リソース名・識別子は原語を維持する。根拠にログを含める場合は§7-10のマスキング後の要約のみ(Slack/GitHub投稿前に適用)。
...
8. **Slack対話(I6)の追加ガード**: 署名+5分timestampを検証後、公開slack ingress自身が`ALLOWED_SLACK_TEAM_IDS` / `ALLOWED_SLACK_CHANNEL_IDS` / `ALLOWED_SLACK_USER_IDS`の3つすべてをenqueue前に検証する。未設定・空・不一致はDB登録/Task作成をせず固定200 ACKで終了し、Slack再送による増幅も防ぐ。既存`event_id`の冪等判定後、新規eventだけをuser単位の設定済み1分上限(例:5。未設定/不正値はdefault-deny)で制限し、`team_id+channel_id+user_id`のtransaction advisory lock下で直近`slack_events`件数確認とevent登録を直列化する。上限超過もTaskを作らず固定200 ACKと安全なcounter metricだけを残す。許可された`event_id`と決定論的Cloud Tasks名を原子的に登録し、Task enqueue後だけ200 ACKする。channel allowlistは一次通知先と一致させ、user allowlistはon-call/運用担当だけを登録する。workerもSlack複合キー、未解決状態、GitHub Issue、3 allowlist、incidentのresource allowlistを実行直前に再検証し、lease+最大3回で処理する。
...
10. **出力サニタイズ**: アラート受信直後、incident_key算出・DB永続化・embedding API送信より前に、`raw_alert`とembedding入力を自動マスキングする。JSONは許可フィールドだけを再帰処理し、未知フィールドを出力へ引き継がない。ログは行単位でBearer/Basic/JWT、cookie、email、資格情報入りURI、PEM鍵、接続文字列、secret/password/token/api-key代入、既知prefix(`AIza`,`AKIA`/`ASIA`,`gh[pousr]_`,`xox[baprs]-`,`sk-`)および安全metadata allowlistに該当しない32文字以上の高entropy列を決定論的に`[REDACTED]`へ置換する。ツール出口、embedding/モデル/Session投入前、outbox保存前、外部送信直前の各境界で同じscannerを再適用し、置換後にresidual scanを行う。検知器例外、未処理match、サイズ/走査上限超過は内容全体を破棄してfail-closedとし、検知ログには種別/counterだけを残して原値・hashを記録しない。Slackのログ由来/LLM生成本文はBlock Kit `plain_text`で送り、`<`/`>`/`&`や`<!here>`/`<@U...>`/`<#C...>`をmrkdwnとして解釈させない。リンクが必要な箇所だけ別要素でallowlist検証済みURLとescape済みlabelから構築する。GitHub MarkdownもHTML/mention/link制御文字をescapeし、未知URLは`[LINK REMOVED]`、表示ラベルと遷移先の不一致は禁止する。Cloud Traceには生ログ・ツール結果・プロンプトを記録しない。マスキング例外・必須フィールドの処理不能・出力検証失敗は必ずfail-closedとする。受信時なら生入力を破棄し、`messageId/resource/alert_policy/masking_error=true`だけの固定安全JSONからfallback incident_keyを算出して`embedding_unavailable=true,status='escalated'`行+定数だけの固定outboxへ収束させ、embedding/LoopAgentを呼ばない。ツール/Session/outbox段階なら当該内容を保存・送信せずincidentをescalatedへ遷移し、同じ固定outboxだけを使う。既知prefix・非標準key/value・高entropy・分割token・scanner例外を契約fixtureに含める。
11. **component別secret/IAM分離**: 共有イメージへsecretを焼き込まず、Cloud Run実行SA・Secret Manager `secretAccessor`・Cloud SQL DB roleを3サービス+dispatcher Job別にする。ingestは`ops_ingest` DB role+Monitoring/Logging/Vertex権限だけで、Slack Bot/GitHub/Signing Secretとoutbox queue enqueuerを持たない。公開slackはSigning Secret+Slack queue限定enqueuer+`ops_slack_ingress` role(`slack_events`の必要最小操作のみ)とし、Bot/GitHub token、Vertex/Monitoring/Logging権限、他ops表の更新権限を持たない。workerだけが`ops_worker` role、Bot Token、GitHub token、I6用Monitoring/Logging/Vertex権限を持ち、Signing Secretを持たない。dispatcherは`ops_dispatcher` SELECT role+outbox queue限定enqueuerだけを持ち、全external/Vertex secretを持たない。デプロイ後に各SAから非許可secret参照と非許可DB/API操作が拒否されることを検証する。
12. **公開Slackエッジ防御**: `incident-agent-slack`は外部Application Load Balancer+serverless NEG経由だけを許可し、Cloud Run ingressを`internal-and-cloud-load-balancing`にして既定URLへの直接インターネット到達を拒否する。Cloud Armorで署名検証前のper-IP rate limitと全体RPS上限を強制する。アプリも`Content-Length`とstreaming readの双方でraw bodyを256 KiB以下に制限し、超過・不正lengthはJSON parse/DB/署名計算前に拒否する。edge/アプリ制限の負荷試験と、LB経由の正規Slack署名が通るE2Eを必須とする。
...
- Slack一次通知はBlock Kitで重大度・確信度・調査回数・Playbook、仮説、根拠、推奨アクションを区分表示する。Cloud Traceボタンを表示し、GitHub Issue作成完了後は元のSlackメッセージを冪等更新してIssueボタンを追加する。ボタンにはallowlist検証済みURLだけを使用し、推論過程を追跡可能にする。
...
1. 認証・resource allowlist・alert_policy正規化・messageId形式検証後、**マスキング前に**messageIdを冪等化する。新規deliveryは生入力を含まない`sanitized_alert={"masking_pending":true}`と安全metadata由来fallback `incident_key`で`received`挿入する。既存messageIdはpayloadを再解釈せずDB checkpointだけに従う。`completed`は2xx、`embedding`は手順4、`is_owner=true AND incident_id IS NOT NULL AND status='processing'`かつ紐づくincidentが`investigating`の場合だけowner再開経路へ進む。owner leaseが有効なら別実行中として更新せず5xx、期限切れならL1〜L5・予算予約・incident作成を再実行せず手順8のCASで再取得し、既存incident/deliveryのcanonical dataから共通調査を再開する。`received`かつ正規化済みなら手順2、`masking_pending`のままなら原文を再採用せず§7-10の固定安全escalation+delivery completedへ収束する。その他の不整合状態は更新せず固定escalation metricを出して5xxとする。新規行だけマスキングし、成功時はtoken一致CASで実incident_key/sanitized_alertへ更新する。失敗時は§7-10の安全escalated incident+固定outbox作成とdelivery completedを同一transactionで行い、再送はcompletedとして2xxに収束する。
...
3. L3閾値に達したらL1/L2候補への個別集約を行わず、通常フローから**排他的にストーム分岐**する。`{"alert_policy":<正規化値>,"bucket_start":<UTCの5分境界>,"kind":"storm","resource":<allowlist検証済み値>,"v":1}`をキー順・空白なしのcanonical JSONにしてSHA-256 hexの`storm_key`を作り、`incident_key`にも同じ値を使う。`storm_key`でadvisory lockを取得し、`incidents_open_storm_key`/`incidents_open_storm_scope`でもストーム行を一意にする。既存行へは集約して終了する。新規時は手順4と同じdelivery checkpointで代表アラートのembeddingを生成し(API中はlockを保持しない)、完了後にlockを再取得して既存ストーム行を再確認する。なお非該当なら同transactionで調査予算予約、deliveryのembeddingをコピーした`incident_kind='storm'`行、delivery ownerを確保し、同じ新UUIDを両token、同じ600秒期限を両leaseへ設定する。同時に、同一resource+alert_policyの未解決normal行を`status='merged'`/`merged_into=<storm id>`へ更新してincident token/leaseを失効させ、それらを参照する`alert_deliveries`の`processing/is_owner`行も`status='completed',is_owner=false,completed_at=now(),work_token=<new UUID>,work_lease_expires_at=NULL`へ原子的に更新する。これにより旧ownerの書き込みCASを拒否し、再送は手順1で2xxへ収束する。Issue既存行にはstorm Issue ref確定後に送る統合コメントと`github_close` outboxを各1件作り、workerはコメント成功後だけcloseする。これらをcommitしてから共通調査を開始する。予算/API失敗は手順6へ収束する。L3ヒット後はL4検索・個別行作成へ進まない。
...
8. ownerのtoken/leaseはincidentとdeliveryへ同じ値をmirrorする。absolute deadlineはrequest開始から540秒、各LoopAgent実行枠は残時間内で最大270秒、leaseは600秒。期限切れ、incidentが`investigating`、deliveryが`processing/is_owner`、かつincident `attempt_count < 3`の場合だけ、両方の旧token一致をCAS条件に新UUIDを両tokenへ設定し、両leaseを同じ期限へ延長し、incident attempt+1を同一transactionで行う。CAS成功した再送ownerだけが、保存済みembedding/canonical alertとincidentのalert_policyから固定選択したplaybookでLoopAgentを再実行する。片方だけの更新は禁止し、古いtokenの書き込みを拒否する。attempt=3なら加算せず手順9を同じCAS transactionで実行する。
9. 成功時は両token一致+incident `investigating`+delivery `processing/is_owner`をCAS条件に、分析+outbox+analyzed+delivery completedを同一transactionで保存する。CAS失敗時は結果を破棄して再読込し、storm統合済みの`merged` incident+`completed` deliveryなら2xx、それ以外は5xxとする。3回目の調査失敗または3回目のlease失効時も同じCAS条件で、`attempt_count=3`のまま固定文outbox+escalated+delivery completedを原子的に保存し、CHECK上限を超える更新を行わない。
...
**外部出力のoutbox**: Slack/GitHub書き込みは`ops.outbox`を経由する。commentは対応する`github_issue` outboxを、closeは直前の統合comment outboxを`depends_on`で参照し、依存先`sent`までdispatchしない。Task bodyは`outbox.id`だけを識別子として受け、workerはdestination/payload/incident/idempotency_key/dependencyを必ずDB行から再読込する。bodyの追加fieldやDB不一致は拒否し、DB行を唯一の正本としてlease+idempotency keyで送信する。Slack一次投稿はIncoming Webhookでなく専用Bot Tokenの`chat.postMessage`を使い、成功レスポンスの`channel`と`ts`を`incidents.slack_channel/slack_thread`へ保存し、設定済みteam IDを`slack_team`へ保存する。GitHub Issue成功時はoutbox `external_ref`と`incidents.github_issue`を同一transactionで保存する。I6は4フィールドがすべて揃うまでfail-closed。失敗は最大10回再試行後`failed`として運用アラートを出す。

**outbox dispatcher**: Cloud Schedulerが1分ごとに専用Cloud Run Job `incident-agent-outbox-dispatcher`を起動する。Jobは`status='pending'`かつ`depends_on IS NULL OR dependency.status='sent'`の行だけを読み、Task名`outbox-<id>-<dispatch_generation>`、body `{"outbox_id":"<id>"}`でoutbox queueへenqueueする。`ALREADY_EXISTS`は成功扱い、その他のenqueue失敗は行を変更せず次回scanで再試行する。このためingest→Cloud Tasksの非原子的dual-writeを行わず、Issue ref確定待ちcomment/storm closeも依存outbox送信後に必ず再scanされる。dispatcher SAはqueue限定enqueuerと`ops_dispatcher` DB role(outbox/incident/dependencyの必要最小SELECT)だけを持ち、Bot/GitHub/Signing/Vertex secret・権限を持たない。

**failed outboxの回復**: 原因修正後、運用者専用CLI `replay_outbox <outbox UUID>`で`status='failed'`行だけを`pending`、`attempt_count=0`、`lease_expires_at=NULL`へCAS更新し、`dispatch_generation`を1増やす。destination/payload/idempotency_key/depends_onは変更不可とし、同じidempotency key、新しいTask名で再送する。CLIはops operator用DB資格情報を使い、AgentランタイムSA・公開エンドポイントから呼べないようにする。Slack成功/GitHub失敗などの部分成功時も、欠けたoutboxだけを再実行してincident参照を補完し、4参照が揃うまでI6は拒否を続ける。
...
- **スレッド履歴の最小scope**: `conversations.replies`用scopeは通知先channel種別に合わせ、public channelなら`channels:history`、private channelなら`groups:history`の一方だけを付与する。Botを設定済みchannelのmemberにし、team/channel allowlist外の履歴は取得しない。DM/MPIMを通知先にせず、`im:history`/`mpim:history`は付与しない。
...
- **主体allowlistとingress rate**: team/channel/userの3種類を設定で明示し、すべてdefault-denyとする。slack ingressは新規eventのDB登録/Task enqueue前に3種類を検証し、不一致は固定200 ACKのみで終了する。許可eventにも§7-8の原子的なuser単位1分上限を適用する。通知先channelとon-callユーザーだけを許可し、workerでも再検証する。incidentのresource allowlistも再検証する。
...
- Slack Eventsは外部Application Load Balancer+serverless NEG+Cloud Armor経由だけでCloud Run `incident-agent-slack`へ到達させ、Cloud Run ingressを`internal-and-cloud-load-balancing`に限定する。per-IP/全体rateと256 KiB body上限を§7-12どおり検証
...
- Slack/GitHub outbox配信用Cloud Tasks queue。ingest/slack SAにはこのqueueのenqueuerを付与しない
- Cloud Scheduler→専用Cloud Run Job `incident-agent-outbox-dispatcher`を1分間隔で起動。dispatcher SAだけにoutbox queue限定`roles/cloudtasks.enqueuer`と`ops_dispatcher`最小SELECT roleを付与
...
- incident-agent 用実行SAをingest/slack/worker/dispatcher別に作成し、§7-11と§10のIAM/secret allowlistを適用
- incident-agent 用Cloud Run 3サービス(ingest/slack/worker)。共有イメージ・別entrypoint、デプロイパイプラインはアプリと分離
- Cloud SQLに`ops_ingest`/`ops_slack_ingress`/`ops_worker`/`ops_dispatcher`のcomponent別DB roleを作成し、slack ingressは`slack_events`必要最小操作、dispatcherはeligible outbox判定用SELECTだけに限定
- Slack App作成(I6用): Bot Token Scopes(`app_mentions:read`, `chat:write`に加え、通知先がpublicなら`channels:history`、privateなら`groups:history`の一方のみ)+ Events APIのリクエストURL設定。Botを通知先channelへ参加させ、DM/MPIM scopeは禁止
...
| --- | --- | --- | --- | --- |
| I1 | opsスキーマ+incidents/alert_deliveries/slack_events/outbox/budget_usage | なし | `services/incident-agent/db/**` | outbox depends_on/generation含む全制約、budget予約、4 component最小DB role |
...
| I3 | LoopAgent+3ツール+プレイブック注入 | I2 | `services/incident-agent/agent/**`, `playbooks/**` | reviewed事例のみ。resource/60分固定、playbook containment、各境界secret scan fail-closed |
| I4 | outbox dispatcher+Slack/GitHub worker+RCAレビュー/replay CLI | I3, §14 | `services/incident-agent/app/**`, `services/incident-agent/jobs/**`, `services/incident-agent/scripts/**` | dependency/generation scan→Task、worker DB正本、chat.postMessage refs、storm close、replay |
...
| I6 | Slack対話型二次切り分け(app_mention→session=incident UUID→追調査→スレッド返信+issueコメント追記) | I4, §14 | `services/incident-agent/app/**` | LB/Armor/body上限、Signing+allowlist/rate、worker DB正本、Session secret scan+TTL |
