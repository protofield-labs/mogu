...
1. 認証・resource allowlist・alert_policy正規化・messageId形式検証後、**マスキング前に**messageIdを冪等化する。新規deliveryは生入力を含まない`sanitized_alert={"masking_pending":true}`と安全metadata由来fallback `incident_key`で`received`挿入する。既存messageIdはpayloadを再解釈せずDB checkpointだけに従う。`completed`は2xx、`embedding`は手順4、`is_owner=true AND incident_id IS NOT NULL AND status='processing'`はowner再開経路へ進む。owner leaseが有効なら別実行中として更新せず5xx、期限切れならL1〜L5・予算予約・incident作成を再実行せず手順8のCASで再取得し、既存incident/deliveryのcanonical dataから共通調査を再開する。`received`かつ正規化済みなら手順2、`masking_pending`のままなら原文を再採用せず§7-10の固定安全escalation+delivery completedへ収束する。新規行だけマスキングし、成功時はtoken一致CASで実incident_key/sanitized_alertへ更新する。失敗時は§7-10の安全escalated incident+固定outbox作成とdelivery completedを同一transactionで行い、再送はcompletedとして2xxに収束する。
