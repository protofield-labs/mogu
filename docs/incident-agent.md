...
Slackスレッドで @incident-agent に追加質問(I6)
  ...
  → team_id+channel+thread_tsでincident照合 → session_id = ops.incidents.id(Vertex AI Sessions)
...
8. **Slack対話(I6)の追加ガード**: `X-Slack-Signature` / `X-Slack-Request-Timestamp` による署名検証を最初のゲートとし、timestampは現在時刻から5分以内のみ許可する(失敗時401・処理しない)。署名検証後、非同期処理を起動する前に`event_id`を`ops.slack_events`へ原子的に挿入し、既存なら200 ACKのみで終了する。`team_id`+`channel`+`thread_ts`を`slack_team`+`slack_channel`+`slack_thread`で照合し、**行なし / `github_issue IS NULL` / 解決済み / analyzed・escalated以外**なら調査せず固定応答で終了する。Vertex AI Sessionsの`session_id`はSlack timestampでなく照合済みの`ops.incidents.id`を使う。ユーザーallowlistはdefault-denyとし、空設定・未登録ユーザーを拒否する。
...
| --- | --- | --- | --- | --- |
| I1 | opsスキーマ+incidents/slack_eventsテーブル+シード事例 | なし | `services/incident-agent/db/**` | §3のDDL通り。調査予約/集約カラム、予約時embedding、Slack team/channel/thread複合キー、event_id冪等化テーブルを含む |
...
| I6 | Slack対話型二次切り分け(app_mention→session=incident UUID→追調査→スレッド返信+issueコメント追記) | I4, §14のSlack App | `services/incident-agent/app/**` | event_id dedupe。team/channel/thread照合後のincident UUIDをsession_idに使用。未解決のみ/default-deny/URLサニタイズ |
