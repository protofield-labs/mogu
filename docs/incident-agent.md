...
Cloud Monitoring アラートポリシー発火
  ...
  ▼
Pub/Sub ──push──▶ Cloud Run service「incident-agent」(アプリとは別サービス)
                  ...
                  一次切り分け結果(仮説・重大度・推奨アクション・根拠・確信度)
                     ├─ Cloud SQL "ops" スキーマへ保存(生アラート+分析+埋め込み)
                     ├─ Slack 通知(スレッド起点)
                     └─ GitHub Issue 起票(ラベル: incident)

Slackスレッドで @incident-agent に追加質問(I6)
  → app_mention → 対話エンドポイント → session_id = thread_ts(Vertex AI Sessions)
  → 同じ読み取り専用ツールで追調査 → スレッドに返信(対話型二次切り分け)
```

### 分離の原則(別Cloud Runサービスとする理由)

**「別サービス」なのは incident-agent と mogu本体アプリ(dev-web)の関係を指す。** ADK LoopAgentは incident-agent サービスのコンテナ内で動くプロセス(ライブラリのクラスをインスタンス化して呼ぶ)であり、別のCloud Runサービス・別デプロイ単位ではない。デプロイ単位は「dev-web」「incident-agent」の2つ。
...
CREATE TABLE ops.incidents (
  ...
  incident_key   text NOT NULL UNIQUE,   -- hash(service+host+message)。冪等化・完全一致キー(§9 L1)
  alert_policy   text NOT NULL,
  resource       text NOT NULL,          -- 例: cloud_run/dev-web
  severity       text NOT NULL,          -- agent判定: critical/high/medium/low
  ...
  status         text NOT NULL DEFAULT 'analyzed', -- analyzed/escalated/resolved
  github_issue   text,
...
| tool | 中身 | 制限 |
| --- | --- | --- |
| `get_metrics` | Cloud Monitoring API(timeSeries.list) | 対象リソース・直近60分に限定 |
| `get_logs` | Cloud Logging API(entries.list) | 対象サービス・severity>=WARNING・最大200行。出力は§7-10のマスキングを経由 |
| `search_similar_incidents` | ops.incidents をpgvector cos類似検索 | 上位3件 |
...
1. **読み取り専用をIAMで強制**: AgentのSAには `roles/monitoring.viewer` / `roles/logging.viewer` / `roles/cloudtrace.agent`(§8のトレース送出用・Traceへの書込のみ)+opsスキーマDBロール+Slack webhook+GitHub fine-grained token(issue作成のみ)。本番リソースへの書き込み系・デプロイ系ロールは一切付与しない。プロンプトが乗っ取られてもIAMが最後の壁。
...
8. **Slack対話(I6)の追加ガード**: `X-Slack-Signature` / `X-Slack-Request-Timestamp` による署名検証を最初のゲートとする(失敗時401・処理しない)。bot_idを持つメッセージは無視(ボット同士のループ防止)/ユーザーallowlist/スレッド対話にも4.の予算上限を適用。
9. **受信エンドポイントの認証**: Pub/Sub pushはOIDC JWT検証必須。Cloud Runはunauthenticated禁止。push用SAのみ `run.invoker`。認証失敗時はfail-closed(調査・Issue起票なし)。
10. **ログ秘匿化**: ツール出力およびSlack/GitHub投稿前に自動マスキング(Bearer/JWT・cookie・email・接続文字列パターン等)。該当箇所は `[REDACTED]` に置換。人間向けRCAではログ全文引用を避け、要約+トレースID参照を優先。

## 8. 可観測性

- エージェント自身の推論トレース・ツール呼び出し・トークンコストをCloud Traceへ送出。incident-agentは`adk web`起動ではなくPub/Sub pushを受けるカスタムサービスのため、CLIフラグ(`--otel_to_cloud`)ではなくADKのtelemetryモジュールでプログラム的に設定する(`get_gcp_exporters(enable_cloud_tracing=True)` + `maybe_set_otel_providers`)。
...
| 層 | 判定 | コスト | ヒット時の動作 |
| --- | --- | --- | --- |
| **L1 ハッシュ完全一致** | `hash(service+host+message)` = `incident_key` が既存と一致か(決定的・完全一致) | ほぼ無料 | `ON CONFLICT DO NOTHING`。既存へ集約し即ACK終了 |
| **L2 グルーピング窓** | 同一リソースの直近15分に既存インシデントがあるか | 無料 | 新規調査せず、既存issueにコメント追記 |
...
| **L4 embedding類似**(Phase A) | 新アラートのembeddingが直近の未解決インシデント(`resolved_at IS NULL`)とcos類似か(閾値超) | 中(埋込計算) | 新issueを作らず既存issueに「関連アラート検知」コメント |
...
- **L1 ハッシュ(完全一致)**: `service|host|message` を固定長の指紋に変換し、指紋の一致だけを見る。hashは決定的(同じ入力→毎回同じ指紋)なので、同一文言・同一ホストの連打を無料で集約できる。弱点: 1文字違うと別指紋になり、表現の揺れ(「Connection timeout」と「timed out」)を吸収できない。
...
L1で大半を集約し、すり抜けた「表現違いの同一障害」だけをL4で捕まえる二段構え。L4は既存の`search_similar_incidents`ツール(pgvector)を流用し、追加インフラなしで実装する。
...
- **session_id = Slackスレッドのthread_ts**(Vertex AI Sessions)。スレッド=会話境界となり、外部の状態管理が不要。`ops.incidents.slack_thread` からインシデント文脈を復元。
...
- Pub/Sub topic `incident-alerts` + Cloud Run(incident-agent)へのpushサブスクリプション(OIDC認証付き。§7-9)
- Cloud Run(incident-agent)はunauthenticated禁止。push用SAのみ `roles/run.invoker`
- Cloud Monitoring アラートポリシー + 通知チャネル(Pub/Sub)
- incident-agent 用サービスアカウント(§7のIAMロール)
- incident-agent 用 Cloud Run サービス(デプロイパイプラインはアプリと分離)
...
| --- | --- | --- | --- | --- |
| I1 | opsスキーマ+incidentsテーブル+シード事例 | なし | `services/incident-agent/db/**` | §3のDDL通り。シード数件投入。アプリ用ロールからops不可視 |
| I2 | 受け口(Pub/Sub push受信+L1〜L4ノイズ制御+自己除外+コストブレーカー) | I1, §14 | `services/incident-agent/app/**` | Pub/Sub OIDC JWT検証必須(§7-9)。同一incident_key再送で調査が走らない。同一リソース15分は既存issueへコメント。L4: embedding類似で既存issueへコメント(LoopAgent前・§9)。閾値超でストームモード。自分宛アラートで即終了。日次上限超過でfail-closed |
| I3 | LoopAgent+3ツール+プレイブック注入 | I2 | `services/incident-agent/agent/**`, `playbooks/**` | max_iterations=3。早期終了が機能。出力が§4の固定フォーマット。ログ200行制限 |
| I4 | 出力(Slack通知+GitHub Issue起票+ops保存) | I3 | `services/incident-agent/app/**` | Slackに仮説+根拠(§7-10マスキング済)+issueリンク。新規インシデントのembeddingをopsに保存。issueにincidentラベル |
| I5 | otel_to_cloud+圧縮率メトリクス(受信アラート→インシデント→Issue) | I4 | `services/incident-agent/**`(設定のみ) | Cloud Traceに推論トレースが出る。圧縮率がダッシュボードで見える |
| I6 | Slack対話型二次切り分け(app_mention→session=thread_ts→追調査→スレッド返信+issueコメント追記) | I4, §14のSlack App | `services/incident-agent/app/**` | Slack署名検証必須(§7-8)。即ACK(3秒対策)。bot_id無視。allowlist。§7-4予算適用。発見をissueへコメント(§7-10マスキング済) |
