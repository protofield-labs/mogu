...
5. deliveryにembeddingが保存された後、L4用lockの同一transaction内で再検索する。hitは集約+completed。missは調査予算予約後、`alert_deliveries.embedding`を`incidents.embedding`へ必ずコピーしてincidentを挿入し、delivery owner processingまで原子的に行う。
